...
export default function LeagueCard({ name, description, start, end } : { name: string, description: string, start: Date, end: Date }) {
    return (
        <li className="flex p-2 mb-4 justify-between bg-white rounded-md shadow-md">
            <Link href="#" className="basis-[25%] truncate">{name}</Link>
